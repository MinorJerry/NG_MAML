...
class FiniteDifferenceHvp(Optimizer):
    ...
    def ng_build_graph(self, constraint_obj, params_var, input_val_dict, reg_coeff):
        """
        Sets the objective function and target weights for the optimize function
        ...
        params = [params_var[key] for key in update_param_keys]
        constraint_grads = tf.gradients(constraint_obj, xs=params)
...
class ConjugateGradientOptimizer(Optimizer):
    ...
    def ng_build_graph(self, loss, params_var, input_ph_dict, leq_constraint):
        """
        Sets the objective function and target weights for the optimize function
        ...
        # build the graph of the hessian vector product (hvp)
        self._hvp_approach.ng_build_graph(constraint_objective, params_var, self._input_ph_dict, self._reg_coeff)
    ...
    def descent_direction(self, input_val_dict):
        """
        Carries out the optimization step
        ...
        Hx = self._hvp_approach.build_eval(input_val_dict)
        descent_direction = conjugate_gradients(Hx, gradient, cg_iters=self._cg_iters)
        self.init_step_size = np.sqrt(2.0 * self._max_constraint_val *
                                    (1. / (descent_direction.dot(Hx(descent_direction)) + 1e-8))) # todo
        return descent_direction

    def descent_direction_given_grad(self, input_val_dict, gradient):
        Hx = self._hvp_approach.build_eval(input_val_dict)
        descent_direction = conjugate_gradients(Hx, gradient, cg_iters=self._cg_iters)
        self.init_step_size = np.sqrt(2.0 * self._max_constraint_val *
                                      (1. / (descent_direction.dot(Hx(descent_direction)) + 1e-8)))  # todo
