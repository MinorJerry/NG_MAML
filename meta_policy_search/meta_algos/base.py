...
import numpy as np
from meta_policy_search.optimizers.conjugate_gradient_optimizer import FiniteDifferenceHvp, conjugate_gradients, _flatten_params, _unflatten_params
...
class MAMLAlgo(MetaAlgo):
    ...
    def _ng_make_input_placeholders(self, prefix=''):
        """
        Args:
            prefix (str) : a string to prepend to the name of each variable
        ...
        for task_id in range(self.meta_batch_size):
            phs_dict = OrderedDict()
            # observation ph
            ph = tf.placeholder(dtype=tf.float32, shape=[None, self.policy.obs_dim], name='obs' + '_' + prefix + '_' + str(task_id))
            phs_dict['%s_task%i_%s'%(prefix, task_id, 'observations')] = ph
            obs_phs.append(ph)

            # action ph
            ph = tf.placeholder(dtype=tf.float32, shape=[None, self.policy.action_dim], name='action' + '_' + prefix + '_' + str(task_id))
            phs_dict['%s_task%i_%s' % (prefix, task_id, 'actions')] = ph
            action_phs.append(ph)

            # advantage ph
            ph = tf.placeholder(dtype=tf.float32, shape=[None], name='advantage' + '_' + prefix + '_' + str(task_id))
            phs_dict['%s_task%i_%s' % (prefix, task_id, 'advantages')] = ph
            adv_phs.append(ph)
            ...
            for info_key, shape in dist_info_specs:
                ph = tf.placeholder(dtype=tf.float32, shape=[None] + list(shape), name='%s_%s_%i' % (info_key, prefix, task_id))
                phs_dict['%s_task%i_agent_infos/%s' % (prefix, task_id, info_key)] = ph
                dist_info_ph_dict[info_key] = ph
            dist_info_phs.append(dist_info_ph_dict)
            all_phs_dict.append(phs_dict)
    ...
    def _ng_build_inner_adaption(self):
        """
        Creates the symbolic graph for the one-step inner gradient update (It'll be called several times if
        more gradient steps are needed)
        ...
        for i in range(self.meta_batch_size):
            with tf.variable_scope("adapt_task_%i" % i):
                ...
                # get tf operation for adapted (post-update) policy
                with tf.variable_scope("adapt_step"):
                    hvp = self._adapt_sym(surr_obj_adapt, self.policy.policies_params_phs[i])
                hvps.append(hvp)

        return hvps, adapt_input_ph_dict
    def _build_inner_adaption_norm(self):
        """
        Creates the symbolic graph for the one-step inner gradient update (It'll be called several times if
        more gradient steps are needed)
        ...
        for i in range(self.meta_batch_size):
            with tf.variable_scope("adapt_task_%i" % i):
                ...
                # get tf operation for adapted (post-update) policy
                with tf.variable_scope("adapt_step"):
                    adapted_policy_param = self._norm_adapt_sym(surr_obj_adapt, self.policy.policies_params_phs[i])
                adapted_policies_params.append(adapted_policy_param)
    ...
    def _ng_adapt_sym(self, surr_obj, constraint_objective, params_var, target):
        """
        Creates the symbolic representation of the tf policy after one gradient step towards the surr_obj
        ...
        grads = tf.gradients(surr_obj, [params_var[key] for key in update_param_keys]) #list
        self.ng_gradients = dict(zip(update_param_keys, grads))

        # natural gradient descent
        # Conjugate gradient for computing F^-1 * g, KL->FIM vector product->CG
        hvp_approach = FiniteDifferenceHvp()
        # for each task
        hvp_approach.build_graph(constraint_objective, target, input_val_dict, 0)
        return hvp_approach

    def _norm_adapt_sym(self, surr_obj, params_var):
        """
        Creates the symbolic representation of the tf policy after one gradient step towards the surr_obj
        ...
        grads = tf.gradients(surr_obj, [params_var[key] for key in update_param_keys])
        # print(len(grads)) -> 7
        # print([g.shape for g in grads]) -> weight,bias,weight,bias,weight,bias,std_param
        # grads_norm = tf.sqrt(sum([tf.norm(g) ** 2 for g in grads]))

        grads = [g/tf.norm(g) for g in grads]
        gradients = dict(zip(update_param_keys, grads))
    ...
    def _ng_adapt(self, samples):
        ...
        # adapted_policies_params_vals = sess.run(self.adapted_policies_params, feed_dict=feed_dict)
        for i in range(self.meta_batch_size):
            Hx = self.hvps[i].build_eval(input_dict)


        Hx = self._hvp_approach.build_eval(input_dict) # todo
        gradient = sess.run(self.ng_gradients, feed_dict=input_dict)
        flatten_grad = _flatten_params(gradient)
        descent_direction = conjugate_gradients(Hx, flatten_grad, cg_iters=10)
        descent_direction = _unflatten_params(descent_direction, gradient)
        params = self.policy.get_param_values() # todo match or not
        update_param_keys = list(params.keys())
        adapted_policy_params = [params[key] - tf.multiply(self.step_sizes[key], descent_direction[key])
                                 for key in update_param_keys]
        # store the new parameter values in the policy
        self.policy.update_task_parameters(adapted_policy_params)
    ...
    def _ng_extract_input_dict(self, samples_data_meta_batch, keys, prefix=''):
        """
        Re-arranges a list of dicts containing the processed sample data into a OrderedDict that can be matched
        with a placeholder dict for creating a feed dict
